...
const AccountAccountRefRolesAddForm = ({account, onSelect}:IProp) => {
    ...
    useEffect(() => {
        if (account?.rolesRefs) {
            ...
            const tarnsformedData:IRoleRow[] = roles
                .filter(item => !userRoles.has(item._id || '') && item.scope === 'account')
                .map((item) => {
                    return {
                        _id: item._id || '',
...
export default AccountAccountRefRolesAddForm